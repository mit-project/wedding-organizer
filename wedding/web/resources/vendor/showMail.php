...
$date=date_create($row2['weddingDate']);
$wDate = date_format($date,"M d, Y");
}
...
<body>
    ...
    <a href="reply.php?reply=<?php echo $row2['id'];?>" class="btn btn-info" style="margin-left:600px;">Reply</a>
    <div>&nbsp;</div>
     <div><strong><?php echo $row2['subject']?></strong></div><br>
     <div>From:&nbsp;<?php echo $row2['email']?></div>
     <div>To:&nbsp;<?php echo $row2['vendorMail']?></div><br>
     <div>Dear Sir/Madam,</div><br>
     <div><?php echo $row2['message']?></div><br>
     <div>Wedding Date:&nbsp;<?php echo $wDate;?></div>
     <div>You can contact me through <?php echo $row2['contactMethod'];?>&nbsp;at any time for more details.</div><br>
     <div>Kind Regards,</div>
     <div><?php echo $row2['senderName'];?></div>
     <div><?php echo $row2['phoneNumber'];?></div>
